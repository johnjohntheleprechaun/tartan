...
2. Merge contexts.
    ...
    - The `inheritable context` is created by merging the `context.default` file contents with the parent node's `inheritable context` (if the `inherit` property of the context file is `true`), or the `root context` (if `inherit` is `false`).
    - The `local context` is created by merging the `context` file contents with _this_ node's `inheritable context` (if the `inherit` property of the context file is `true`), or the `root context` (if `inherit` is `false`).
    - Merging is done by overlaying the top level properties. A local context overrides properties from the inheritable context, and the inheritable context overrides properties from the parent's inheritable context.

3. Determine the node type (may have been changed to `handoff`).
