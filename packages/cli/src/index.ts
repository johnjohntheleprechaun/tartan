...
program
    ...
    .option(
        "--config-file <path>",
        "The path to your tartan config file, WITHOUT the file extension",
        "tartan.config",
    )
    .option("-o, --ouput <path>", "The name of the file to output", "out.html")
    .option(
        "-v, --verbose",
        "Whether (and to what level) debug logs should be printed",
        (_: string, prev: number) => prev + 1,
        0,
    )
    .action(async (opts) => {
        const config = await Resolver.loadObjectFromFile<TartanConfig>(
            opts.configFile,
        );
        if (config === undefined) {
